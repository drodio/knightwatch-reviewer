...
2. **Retire any deployed `docker-compose.override.yml`, in the same pre-pull window.** Port **every** mount it declares into `docker/secrets/config.env` — the clone root into `KID_ROOT`, any other index into `KID_EXTRA_MOUNTS` (see [kid prior-art](#kid-prior-art); on the production host today that is `KID_ROOT=/home/odio/services/kwr-repos` plus `KID_EXTRA_MOUNTS=/home/odio/Hacking/plow-kid`, and dropping the second one silently disables prior art on plow). Because each extra index now mounts at the **same path inside the container**, also change prod's `docker/secrets/repos.conf` `KID_PATHS` entry for `["plow-pbc/plow"]` from `/kid-ro/plow-kid` to `/home/odio/Hacking/plow-kid`. Do it *here*, not after step 5: the containers re-source `/shared/repos.conf` per review, so the new value takes effect the instant it lands, while the matching mount only appears on the next render — edit it against a running pre-migration fleet and every plow review silently falls through to `kid index not yet built … — skipping prior-art lookup`. Then delete the override — it stays gitignored so an uncleaned deployment doesn't trip the deploy preflight, but leaving it in place means compose keeps auto-merging a file the generator no longer expects. If the deployment also carries a `config.env.bak-kid`, check whether it holds kid wiring that needs porting alongside the override before deleting either. (Follow-up, not this PR: relocating plow's index under `KID_ROOT` would remove the need for `KID_EXTRA_MOUNTS` altogether.)
...
5. `sudo systemctl restart knightwatch-reviewer.service` — recreates every container against the re-rendered mounts, so mount and manifest agree from the first review onward.
