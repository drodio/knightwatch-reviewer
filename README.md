...
5. Because each extra index now mounts at the **same path inside the container**, change prod's `docker/secrets/repos.conf` `KID_PATHS` entry for `["plow-pbc/plow"]` from `/kid-ro/plow-kid` to `/home/odio/Hacking/plow-kid` — and **immediately** `sudo systemctl restart knightwatch-reviewer.service`, as **one step, never split across a pause**. Either half alone is inconsistent: the containers re-source `/shared/repos.conf` per review so the new value takes effect the instant it lands, while the matching mount only arrives when the restart recreates the containers. Pausing anywhere in between leaves every plow review silently falling through to `kid index not yet built … — skipping prior-art lookup`. (Follow-up, not this PR: relocating plow's index under `KID_ROOT` would remove the need for `KID_EXTRA_MOUNTS`, and this step, altogether.)
