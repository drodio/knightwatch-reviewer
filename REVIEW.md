...
**Stage:** The reviewer reviewing itself. Single-operator tool used by one engineer (srosro). The review loop runs as a containerized multi-account deployment — the **only** review path; the legacy single-account systemd-timer-on-one-host reviewer has been retired (units deleted). The remaining host timers (learn, org-sync, poll — merged /srosro-approve + re-request — kid-refresh, bakeoff) are auxiliary only. Not a product, not a distribution target. Matches § Product context above.
