...
- `.codex-scratch/seed-convention.md` — present ONLY for SEED-convention repos. When present it is **authoritative for how to review this repo**: `SEED.md`/`README.md` (RFC-2119 prose) are the contract, `ref/` is a single-operator reference impl reviewed for prose↔ref drift, and the `## Verification` section / `ref/verify.sh` IS the test gate. Do NOT demand a root `justfile` or a unit-test harness, and do NOT treat a "just test not run (no justfile)" note as a coverage gap — that's the expected shape for a SEED. Absent for non-SEED repos.
