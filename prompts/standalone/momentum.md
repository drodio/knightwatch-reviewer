...
**Operating point (READ FIRST):** Read `.codex-scratch/review.md` before any other input. It is the repo's `REVIEW.md` and carries the per-repo stage, cultural emphasis, voice-posture rules, and review-loop rules; cite `Broken-Glass Test` by name when applying.

**Inputs:**
- `.codex-scratch/review.md` — operating point + review-loop rules (read first; cite Broken-Glass Test).
...
- `.codex-scratch/reeval-status.md` — which deterministic re-eval trigger is live this round (`REEVAL-LOC-TRIGGER`: LOC growth past the PR's opening size (first commit) × 1.33 + 100; the concrete numbers are the `open=`/`current=` values on that line) and whether either trigger has already fired. Name the live trigger's reason in your opening sentence; it's the concrete evidence that the shape warrants re-evaluation.
