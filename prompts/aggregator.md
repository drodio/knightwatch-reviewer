...
3. **Promote the structural probes.** When the banner fires, rank `Class: shape` / `Class: simplification` probes that name the structural cause (including any `[from: architecture-refined]` probe, which arrives as `Class: shape`) ABOVE leaf-level probes of equal severity — the banner and the top probes should point at the same structural thing.
