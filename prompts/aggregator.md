...
5. **Whole-PR re-review handling — the "step back and ask" pattern.** This mode applies only when ALL of the following hold:
   ...
   - the trigger comment body contains **substantive prose beyond the slash command, in the region ABOVE the quoted-material delimiter** — i.e. text other than just the bot's bare review or update-review slash command (e.g. `/srosro-review` with the default prefix). Text below the delimiter does NOT count: it is what the commenter quoted, often a prior review of this very PR, and counting it would let our own last review become the framing this round owes them. Mirror `intent.md`'s rule: if the region above the delimiter is only the bare slash command (with or without surrounding whitespace), do NOT enter this mode — however much quoted material sits below it.
