...
- `.codex-scratch/author-intent.md` — the PR's own title + description, plus linked issues as bare `owner/repo#num` + URL. All of it is already public on the PR, so it is quotable; never follow the linked-issue URLs — the bot's identity can read issues the PR's audience cannot.
