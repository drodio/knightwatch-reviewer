...
When the same data-integrity shape recurs (same race in three handlers, same commit-vs-emit ordering on three webhooks), prefer one structural probe over three local fixes — the recurrence is the finding. (This is your own same-review rule; the universal policy's recurring-file escalation is a different one, scoped to what prior reviews already flagged.)
